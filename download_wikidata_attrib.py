...
def extract_image_license(image_name: str):
    ...
    result = requests.get(start_of_end_point_str +
                          image_name+end_of_end_point_str)
    result = result.json()
...
def extract_wiki_license(image_name):

    start_of_end_point_str = 'https://en.wikipedia.org' \
        '/w/api.php?action=query&titles=File:'
    end_of_end_point_str = '&prop=imageinfo&iiprop=user' \
        '|userid|canonicaltitle|url|extmetadata&format=json'
    result = requests.get(start_of_end_point_str +
